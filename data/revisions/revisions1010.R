...
pdf("shannon_race_case_box.pdf",width=10,height=6)
...
pdf("lcrisp_race_case_box.pdf",width=10,height=6)
...
pdf("liners_race_case_box.pdf",width=10,height=6)
...
write.csv(aldex_rc7,"aldex2_rc.csv")
...
#table S3
write.csv(aldex_rc8,"aldex2_rc_format.csv")
...
write.csv(aldex_rd1[,-c(1:5)],"aldex2_rd.csv")
...
write.csv(aldex_rd7,"aldex2_rd.csv")
...
#table S4
write.csv(aldex_rd8,"aldex2_rd_format.csv")

#Fig.3d
rownames(aldex_rd2)[grep("Gard",rownames(aldex_rd2))]="Gardnerella_spp"
pdf("race_douche.pdf",width=6,height=6)
...
#Fig.3e
pdf("douche_race.pdf",width=6,height=6)
...
write.csv(log_mat,file="table2a.csv")
...
log_mat[2,]="ref"
write.csv(log_mat,file="table2b.csv")
...
valencia=read.csv(file="valencia_results.csv",header=T,row.names=1)
dim(valencia)
table(valencia$CST)
...
sample_cluster=read.csv(file="JF_cluster_group.csv",row.names = 1,header=T)
...
write.csv(cluster3,file="vagitype_CST_comparison.csv")

stir_norm1=read.csv(file="PIN_microbiome_taxa_count.csv",header=T,row.names=1)
...
# Fig. S7b
pdf("pcoa_CST.pdf",width=8,height=8)
stir_norm_d1=log10(stir_norm1+1)
gen_pcoa=capscale(stir_norm_d1~1,distance="bray")
var_per=round((gen_pcoa$CA$eig/sum(gen_pcoa$CA$eig))[1:6]*100,2)
pcoa_p=paste("PCoA",c(1:6)," (",var_per,"%)",sep="")
...
pdf("CST_shannon_box.pdf",width=8,height=5)
...
write.csv(fisher_cluster_race,file="race_difference_in_CST_composition.csv")
"              I          II          III         IV-A      IV-B       IV-C            V
...
write.csv(fisher_cluster,file="race_difference_for_preterm_ratio.csv")
# I        II       III     IV-A      IV-B      IV-C         V
...
pdf("Elovitz_taxa_verify.pdf",width=10,height=8,onefile = T)
par(mfrow=c(2,3))
j=1
...
pdf(file="class_bar_stratify.pdf",height=50,width=12,onefile=T)
